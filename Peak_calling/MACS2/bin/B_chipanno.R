...
enrichedGO = getEnrichedGO (annotatedPeak, orgAnn = bio.anno,
  maxP = 0.01, multiAdj = T, minGOterm = 10, multiAdjMethod = "BH" )
...
anno <-
  read.delim(paste(result.path, bed.file.name, ".annotated", sep=""), header=F)
anno.table <- table(anno[, 11])
lbls <- paste(names(anno.table)," ",
              anno.table, sep="")
pdf(paste(result.path, bed.file.name, "_peak_dis.pdf", sep=""))
pie(anno.table, labels=lbls, col=rainbow(length(lbls)),
    cex=0.8, radius=0.7,
    main=paste("Distribution of Peaks"))
dev.off()
